...
class Headless
  class VideoRecorder
    ...
    def start_capture
      CliUtil.fork_process("#{CliUtil.path_to('ffmpeg')} -y -r #{@frame_rate} -s #{@dimensions} -f x11grab -i :#{@display} -vcodec #{@codec} -g 600 #{@tmp_file_path}", @pid_file_path, @log_file_path)
      at_exit do
